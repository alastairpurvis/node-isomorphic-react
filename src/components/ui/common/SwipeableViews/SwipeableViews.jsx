...
class SwipeableViews extends Component {
    ...
    handleTouchMove = event => {
        ...
        if (this.isSwiping === undefined) {
            if (event.defaultPrevented) {
                ...
                const
                    dx = Math.abs(this.startX - touch.pageX),
                    dy = Math.abs(this.startY - touch.pageY);
                let isSwiping;

                if (this.props.isVertical) {
                    isSwiping = dx < dy && dy > UNCERTAINTY_THRESHOLD;
                } else {
                    isSwiping = dx > dy && dx > UNCERTAINTY_THRESHOLD;
                }
        ...
        if (this.props.isVertical) {
            this.vx = this.vx * 0.5 + (touch.pageY - this.lastY) * 0.5;
            this.lastY = touch.pageY;
        } else {
        ...
        if (this.props.isVertical) {
            index = this.state.indexLatest + (this.startY - touch.pageY) / this.startWidth;
        } else {
            index = this.state.indexLatest + (this.startX - touch.pageX) / this.startWidth;
        }
    ...
    renderContainer(interpolatedStyle, updateHeight, childrenToRender) {
        const
            ...
            translate = -interpolatedStyle.translate;
        let styleNew

        if (this.props.isVertical) {
            styleNew = {
                WebkitTransform: `translate3d(0, ${translate}%, 0)`,
        ...
        return (
            <div
                style={Object.assign(styleNew, (this.props.isVertical ? styles.verticalContainer : styles.container), containerStyle)}>
                {childrenToRender}
            </div>
        );
    }
    ...
    render() {
        ...
        const translate = isVertical ? indexCurrent * (100 / this.props.children.length) : indexCurrent * 100;
        ...
        return (
            <div
                {...other}
                style={Object.assign({}, (isVertical ? styles.verticalRoot : styles.root), style)}
                {...touchEvents}
            >
